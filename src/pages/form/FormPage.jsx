...
export const FormPage = ({ id, modalRef }) => {
  const api = import.meta.env.VITE_API;
  const token = localStorage.getItem("token");

  const { data: detail } = FetchData({ url: `${api}/admin/pages`, token });
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === "banner" && value === null) return;
      ...
      >
        ...
        {oldBanner && (
          <p className="text-xs text-gray-500 mt-1">
            banner lama: <span className="text-blue-500">{oldBanner}</span>
          </p>
        )}
