...
export const FormCategoryBlog = ({ id, modalRef }) => {
  const api = import.meta.env.VITE_API;
  const token = localStorage.getItem("token");

  const { data: detail } = FetchData({ url: `${api}/admin/category-articles`, token });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = id ? `${api}/admin/category-articles/${id}` : `${api}/admin/category-articles`;
